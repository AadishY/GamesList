...
export default function SharedList({ games, updateFirebaseGame, activeProfile, goBack }) {
  ...
  const pillClass = (active) => `flex-1 sm:flex-none px-6 py-3 sm:px-5 sm:py-2.5 rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest transition-all flex items-center justify-center ${
    active ? 'bg-neon-purple brutal-btn' : 'bg-transparent border-2 border-transparent hover:border-black/20 dark:hover:border-white/20 text-black/70 dark:text-white/50 hover:text-black dark:hover:text-white'
  }`;
    ...
    <div className="flex-1 w-full flex flex-col pt-4 sm:pt-6 animate-in fade-in transition-colors relative">
      <div className="max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 pb-32">
        ...
        <div className="glass-panel-flat p-4 sm:p-5 rounded-[2rem] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-5 mb-8">
          <div className="flex items-center gap-4 pl-1">
            ...
            <div className="flex flex-col">
              <h2 className="text-xl sm:text-2xl font-black tracking-tighter uppercase leading-none text-black dark:text-white">Shared List</h2>
              <p className="text-[10px] text-black/70 dark:text-white/50 font-black uppercase tracking-[0.2em] mt-1.5 ml-0.5">Games for us</p>
            </div>
          </div>
        ...
        {sharedGames.length === 0 ? (
          <div className="text-center py-20 glass-panel rounded-[2rem] border-dashed">
            <Gamepad2 className="w-16 h-16 mx-auto text-black/50 dark:text-white/20 mb-5" />
            <h3 className="text-2xl font-black text-black/60 dark:text-white/60 uppercase tracking-tighter">List is empty</h3>
            <p className="text-black/60 dark:text-white/40 font-black mt-2 text-xs uppercase tracking-widest max-w-[200px] mx-auto leading-relaxed">Add games from your library below.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            <div className="px-3 text-[10px] font-black text-black/55 dark:text-white/40 uppercase tracking-[0.2em] mb-1 pl-4">
              Drag to Reorder
            </div>
      ...
      {canEdit && (
        <button onClick={() => setShowAddModal(true)}
          className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 bg-neon-yellow text-black px-6 py-5 sm:py-4 rounded-2xl sm:rounded-[1.5rem] brutal-btn font-black uppercase tracking-widest text-lg transition-all flex items-center justify-center gap-3 z-50 pointer-events-auto active:scale-95"
        >
