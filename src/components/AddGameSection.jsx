  ...
  return (
    <section className="glass-panel p-5 sm:p-8 relative z-40 mb-6 animate-stagger-enter" style={{ animationDelay: '40ms' }}>
      <h2 className="text-xl sm:text-2xl font-black mb-6 flex items-center gap-3 uppercase tracking-tighter">
        <span className="bg-neon-cyan brutal-btn p-2 rounded-xl border-2 border-black">
          <ListPlus className="w-5 h-5 sm:w-6 sm:h-6 text-black" />
        </span>
        <span className="text-black dark:text-white">Add to {activeProfile}</span>
      </h2>
      ...
      <form onSubmit={handleFetchGameDetails} className="flex flex-col sm:flex-row gap-4 relative z-10" ref={dropdownRef}>
        <div className="relative flex-1 group">
          ...
          {showDropdown && searchResults.length > 0 && (
            <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white dark:bg-[#111116] border-2 border-black dark:border-white/20 rounded-2xl shadow-brutal-lg z-50 max-h-80 overflow-y-auto overflow-hidden overscroll-contain animate-scale-spring">
                  ...
                  >
                    ...
                    <div className="flex flex-col min-w-0 w-full">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className="text-[10px] sm:text-xs font-black bg-black/5 dark:bg-white/10 text-black/60 dark:text-white/60 px-2 py-0.5 rounded-md border border-black/10 dark:border-white/10">
                          {game.released ? game.released.substring(0, 4) : 'TBA'}
                        </span>
                        <span className="text-[10px] sm:text-xs text-black/55 dark:text-white/40 font-black uppercase tracking-widest truncate max-w-[120px] sm:max-w-[200px]">
                          {game.genres?.map(g => g.name).slice(0, 2).join(', ')}
                        </span>
                        {existing && (
                           <span className={`text-[9px] sm:text-[10px] px-2 py-0.5 rounded-md font-black uppercase tracking-widest border-2 ${isAddedByMe ? 'bg-[#ff4a4a] text-black border-black/30' : 'bg-neon-purple text-black border-black/30'}`}>
        ...
        <button type="submit" disabled={loading || isSearching}
          className="bg-neon-pink brutal-btn px-8 py-4 rounded-2xl text-lg font-black disabled:opacity-50 whitespace-nowrap flex items-center justify-center gap-3 active:scale-95"
        >
      ...
      {error && (
        <div className="mt-4 flex items-center gap-3 text-black bg-[#ff4a4a] text-sm font-bold px-4 py-3 rounded-xl border-2 border-black shadow-brutal-sm animate-shake">
