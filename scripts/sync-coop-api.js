...
const OUTPUT_FILE = path.join(PUBLIC_DIR, 'coopgames.json');
...
function extractText(cell) {
  if (!cell) return "";
  if (typeof cell === 'object' && cell.t === 's' && cell.v) return cell.v.toString();
  if (cell.v !== undefined) return cell.v.toString();
  return "";
}

async function sync() {
  console.log("📡 Fetching latest Co-Op Database...");
  ...
  try {
    ...
    console.log(`✅ Successfully generated Co-Op JSON API at: ${OUTPUT_FILE}`);

  } catch (error) {
    console.error("❌ Failed to sync Co-Op database:", error.message);
    process.exit(1);
  }
}
